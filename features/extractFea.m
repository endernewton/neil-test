function extractFea( imageName, featurePath, boxSet, options )
...
%                     clear descriptor
                ...
                case 'ssim'
                    ...
                    indexF = getNearest(descriptor, centers);
                    final = hist(indexF(:),1:dictionarySize);
                    MoreFeatureCell{f,b} = final/sum(final);
                    clear indexF
                case 'texton'
...
%         disp(l);
    ...
    boxes = cat(1,boxSet{:});
    mainFea = single(full(mainFea));
    moreFea = single(full(moreFea));
    save(featurePath,'mainFea','moreFea','boxes');
%     vars = {single(full(mainFea)),single(full(moreFea)),boxes};
%     varNs = {'mainFea','moreFea','boxes'};
%     vars = {mainFea,moreFea,rawFea,boxes};
%     varNs = {'mainFea','moreFea','rawFea','boxes'};
%     saveFS(featurePath,vars,varNs);
else
    rawFea = [];
    for f = 1:lH
        featureName = featureSetH{f};
    ...
    boxes = cat(1,boxSet{:});
    mainFea = single(full(mainFea));
    save(featurePath,'mainFea','boxes');
%     vars = {single(full(mainFea)),boxes};
%     varNs = {'mainFea','boxes'};
%     vars = {mainFea,rawFea,boxes};
%     varNs = {'mainFea','rawFea','boxes'};
%     saveFS(featurePath,vars,varNs);
end
